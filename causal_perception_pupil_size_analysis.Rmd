---
title: Pupil size analysis of causal perception experiment
date: "01/08/2021"
output:
...
sample.data <- read_csv("data/causal_perception_sample_report_data.csv")
```
...
*Plot raw data
```{r}
raw_plot_pupil<-ggplot(data = sample.data %>% filter(condition != "hab")%>%
mutate(condition=fct_recode(condition, "No contact"="exp", "Contact"="con")), aes(x = time.frame, y = RIGHT_PUPIL_SIZE)) +
  ylab("Pupil size") +
  xlab("Time (in ms)") +
...
exp.data.pupil.processed <- sample.data %>%
  ...
  select(subject, time.frame, condition, trial, RIGHT_GAZE_X, RIGHT_GAZE_Y, RIGHT_PUPIL_SIZE_no_blinks) %>%
...
plot.contact.pupil.individual<-ggplot(data = exp.data.pupil.processed.downsampled2%>% filter(condition != "hab"), aes(x = bin_low, y =pupil.base.corrected.binned )) +
  ylab("Pupil size ") +
  xlab("Time (in ms)") +
  ...
  theme(panel.grid.major.x = element_blank(),
        ...
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
        legend.title = element_blank(), legend.position = c(0.65, 0.1),
        legend.text=element_text(size=12))

plot.contact.pupil.individual
...
plot.contact.pupil <- ggplot(data = pupil.group.level %>% filter(condition != "hab"), aes(x = bin_low, y = mean.pupil.corrected.binned)) +
  geom_vline(aes(xintercept=baseline.end), lty=2, alpha=0.3)+
    ylab("Pupil size") +
  xlab("Time (in ms)") +
...
ggsave(plot.contact.pupil, filename = "graphics/plot.contact.pupil_gaze_corrected.png", height = 5, width = 8, scale = 0.7)
```
### GAMM
...
plot_diff_m2<-plot_diff(m2, view="time", 
          comp=list(condition=c("exp", "con")), rm.ranef=TRUE, 
...
plot_diff_m2_gg<-ggplot(data=plot_diff_m2, aes(x=time, y=est))+
  ...
  theme_bw()+
  scale_x_continuous(name="Time (in ms)", breaks=c(1000, 2000, 3000, 4000))+ ylab("Est. difference in pupil size")+ggtitle("No contact - Contact")+
    theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
...
ggsave(plot_diff_m2_gg, filename = "graphics/causal_perception_diff_m2_gg.png", width=6, height=6, scale=0.6)

```

* plotting partial effects
```{r}
pdf("graphics/causal_perception_GAMM_partical_effect.pdf", width=10, height = 8)
plot(m2, pages = 1, all.terms = TRUE, residuals = TRUE, rug=FALSE, pch = 1, cex = 0.5, seWithMean = TRUE, shade = TRUE, shade.col = "lightblue")
dev.off()

library(mgcViz)
b <- getViz(m2)
png("graphics/causal_perception_GAMM_partical_effect2.png", units="cm", res=600, width=24, height = 16)
...
```{r}
save.image(file = "causal_perception_GAMM_workspace.RData")
```
...
pg_contact<-plot_grid(plot.contact.pupil, plot_diff_m2_gg, rel_widths = c(2,1),labels=c("E", "F"))

ggsave(pg_contact, filename = "graphics/causal_peception_pupil_size.png", width=10, height=4, scale=0.8)
...
ggsave(pg.contact3, filename = "graphics/causal_perception_figure1.png", width = 10.5, height=13, scale=0.75)
